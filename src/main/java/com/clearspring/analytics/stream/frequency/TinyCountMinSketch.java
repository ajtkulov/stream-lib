 ...
 */
public class TinyCountMinSketch implements IFrequency, Serializable {
    ...
    public TinyCountMinSketch(int depth, int width, int seed) {
        this.depth = depth;
        this.width = width;
        this.eps = 2.0 / width;
    ...
    public TinyCountMinSketch(double epsOfTotalCount, double confidence, int seed) {
        // 2/w = eps ; w = 2/eps
    ...
    TinyCountMinSketch(int depth, int width, long size, int[] hashA, int[][] table) {
        this.depth = depth;
        this.width = width;
        this.eps = 2.0 / width;
        this.confidence = 1 - 1 / Math.pow(2, depth);
    ...
    public boolean equals(Object o) {
        ...
        final TinyCountMinSketch that = (TinyCountMinSketch) o;
     ...
     */
    public static TinyCountMinSketch merge(TinyCountMinSketch... estimators) throws CMSMergeException {
        TinyCountMinSketch merged = null;
        if (estimators != null && estimators.length > 0) {
            int depth = estimators[0].depth;
            int width = estimators[0].width;
            int[] hashA = Arrays.copyOf(estimators[0].hashA, estimators[0].hashA.length);

            int[][] table = new int[depth][width];
            long size = 0;

            for (TinyCountMinSketch estimator : estimators) {
            ...
            merged = new TinyCountMinSketch(depth, width, size, hashA, table);
        }

        return merged;
    }

    public static byte[] serialize(TinyCountMinSketch sketch) {
        ...
        try {
            ...
            for (int i = 0; i < sketch.depth; ++i) {
                s.writeInt(sketch.hashA[i]);
                for (int j = 0; j < sketch.width; ++j) {
                    s.writeInt(sketch.table[i][j]);
                }
            }
            s.close();
    ...
    public static TinyCountMinSketch deserialize(byte[] data) {
        ByteArrayInputStream bis = new ByteArrayInputStream(data);
        DataInputStream s = new DataInputStream(bis);
        try {
            TinyCountMinSketch sketch = new TinyCountMinSketch();
            ...
            sketch.hashA = new int[sketch.depth];
            sketch.table = new int[sketch.depth][sketch.width];
            for (int i = 0; i < sketch.depth; ++i) {
                sketch.hashA[i] = s.readInt();
                for (int j = 0; j < sketch.width; ++j) {
